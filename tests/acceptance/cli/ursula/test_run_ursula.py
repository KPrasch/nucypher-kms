...
from nucypher.utilities.networking import UnknownIPAddress, LOOPBACK_IP
...
def test_run_lone_federated_default_development_ursula(click_runner):
    ...
    assert "{}:{}".format(LOOPBACK_IP, deploy_port) in result.output
...
def test_federated_ursula_learns_via_cli(click_runner, federated_ursulas):
    ...
    assert f"{LOOPBACK_IP}:{deploy_port}" in result.output

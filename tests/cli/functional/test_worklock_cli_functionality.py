...
from tests.constants import (CLI_ENV, MOCK_PROVIDER_URI, YES, NO)
...
def test_enable_claiming(click_runner,
    ...
    result = click_runner.invoke(worklock, command, input=user_input, env=CLI_ENV, catch_exceptions=False)
    assert result.exit_code == 0

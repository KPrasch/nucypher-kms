...
def __execute(compiler_version: VersionString, input_config: Dict, allow_paths=None):
    ...
    allow_paths = f'{SOLIDITY_SOURCE_ROOT},{TEST_SOLIDITY_SOURCE_ROOT}'
    if allow_paths:
        allow_paths += ','.join(str(p) for p in allow_paths)

    # Execute Compilation
    try:
        compiler_output = compile_standard(input_data=input_config, allow_paths=allow_paths)
